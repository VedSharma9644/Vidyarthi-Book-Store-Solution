...
class ApiService {
  ...
  // Upload profile image - separate from admin panel upload
  async uploadProfileImage(userId, fileUri) {
    try {
      ...
      const response = await axios.post(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.USERS.UPLOAD_PROFILE_IMAGE}/${userId}/profile-image`,
        formData,
        {
          headers,
          transformRequest: (data) => {
      ...
      return {
        ...
        url: response.data.url || response.data.data?.imageUrl,
        message: response.data.message || 'Profile image uploaded successfully',
        data: response.data.data || response.data,
      };
    } catch (error) {
      console.error('Upload profile image API Error:', error.message);
      console.error('Upload error details:', error.response?.data);
      throw error;
    }
  }
